...
int packedSorted_search(int a, int left, int right) { //Binary Search
	if (right == left) {
...
int main() {
	...
	while (ch != 'q') {
		...
		if (ch == 'i') {
			...
			else if (packedSorted_search(a, 0, index) != -1) {
			...
			int index_a{};
			index_a = packedSorted_search(a, 0, index);
			...
			int result;
			result = packedSorted_search(a, 0, index);
